...
public final class PhoneNumberKit: NSObject {
    ...
    public func parse(_ numberString: String, ignoreType: Bool = false) throws -> PhoneNumber {
        var numberStringWithPlus = numberString

        do {
            return try self.parseManager.parse(numberString, withRegion: "RU", ignoreType: ignoreType)
        } catch {
        ...
        return try self.parseManager.parse(numberStringWithPlus, withRegion: "RU", ignoreType: ignoreType)
    }
    ...
    public func parse(_ numberStrings: [String], ignoreType: Bool = false, shouldReturnFailedEmptyNumbers: Bool = false) -> [PhoneNumber] {
        return self.parseManager.parseMultiple(numberStrings, withRegion: "RU", ignoreType: ignoreType, shouldReturnFailedEmptyNumbers: shouldReturnFailedEmptyNumbers)
    }
    ...
    public func isValidPhoneNumber(_ numberString: String, withRegion region: String = PhoneNumberKit.defaultRegionCode(), ignoreType: Bool = false) -> Bool {
        return (try? self.parse(numberString, ignoreType: ignoreType)) != nil
    }
    ...
    public func getExampleNumber(forCountry countryCode: String, ofType type: PhoneNumberType = .mobile) -> PhoneNumber? {
        ...
        do {
            return try example.flatMap { try parse($0, ignoreType: false) }
        } catch {
